...
public abstract class TrackGenreAddedConsumerBase<TTrack> : IConsumer<TrackGenreAdded>
    where TTrack : ITrackMongo, new()
{
    private readonly IRepository<TTrack> _repository;

    protected TrackGenreAddedConsumerBase(IRepository<TTrack> repository)
    {
        _repository = repository;
    }
    
    public virtual async Task Consume(ConsumeContext<TrackGenreAdded> context)
    {
        ...
        track.Genres.Add(message.GenreId);
